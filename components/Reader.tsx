...
const Reader: React.FC<ReaderProps> = ({ verse, selectedWord, onSelectWord }) => {
  if (!verse) {
    return (
  ...
  return (
    <div className={`flex-1 p-4 sm:p-8 lg:p-12 overflow-y-auto flex flex-col items-center ${isHebrew ? 'rtl' : 'ltr'}`}>
      <div className="max-w-4xl w-full">
        ...
        <div 
          className={`flex flex-wrap gap-x-3 sm:gap-x-4 gap-y-4 sm:gap-y-6 leading-relaxed ${isHebrew ? 'flex-row-reverse text-right' : 'flex-row text-left'}`}
